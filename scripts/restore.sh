...
docker compose exec -T db sh -c 'exec pg_restore -U "$POSTGRES_USER" -d "$POSTGRES_DB" --clean --if-exists --no-owner --single-transaction' < "$SOURCE/database.dump"
docker compose run --rm -T --no-deps --entrypoint sh app -c 'find /data/documents -mindepth 1 -delete'
docker compose run --rm -T --no-deps --entrypoint tar app -C /data --no-same-owner -xzf - < "$SOURCE/documents.tar.gz"
