...
def main():
    ...
    backup=root/('.env.backup-'+stamp);shutil.copyfile(target,backup);os.chmod(backup,0o600)
    fd,tmp=tempfile.mkstemp(prefix='.env-upgrade-',dir=root)
    try:
        os.chmod(tmp,0o600)
    ...
    print('Execute: docker compose up -d --build; docker compose logs --tail=100 app worker')
