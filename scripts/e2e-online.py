...
try:
    ...
    with sync_playwright() as pw:
        ...
        admin.locator('aside').get_by_role('link',name='Financeiro / ASAAS',exact=False).click()
        expect(admin.get_by_role('heading',name='Conexões desta escola')).to_be_visible();admin.evaluate('window.scrollTo(0,0)');admin.wait_for_timeout(120);admin.screenshot(path=str(OUT/'05-integracoes.png'),full_page=True);record('Painel bancário ASAAS acessível, separado da Connect API; sem ativação automática')
