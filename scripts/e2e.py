...
try:
    ...
    with sync_playwright() as pw:
        ...
        nav('Alunos');page.get_by_role('button',name='+ Novo aluno').click();page.wait_for_timeout(100)
        student_dialog=page.locator('[role="dialog"]')
        print(json.dumps({'student_dialog_count':student_dialog.count(),'student_dialog_text':student_dialog.inner_text()[:1200] if student_dialog.count() else 'sem modal'},ensure_ascii=False))
        student_dialog.screenshot(path=str(OUT/'student-dialog-open.png'))
