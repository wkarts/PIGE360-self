...
try:
    ...
    with sync_playwright() as pw:
        ...
        dialog().get_by_label('Nome completo').fill('Pessoa Unificada — Teste')
        dialog().get_by_label('Tipos de pessoa').select_option(['guardian','staff'])
        save()
        ...
        dialog().get_by_label('Nome completo').fill('Pessoa Unificada Atualizada — Teste');save()
        if BRIDGE:
        ...
        dialog().get_by_label('Nome de apresentação da escola').fill('Colégio Exemplo — Identidade Local')
        dialog().get_by_label('Nome curto no aplicativo').fill('Exemplo Local')
        dialog().get_by_label('Cor principal').fill('#5a1623')
        dialog().get_by_label('Tipografia',exact=False).select_option('georgia')
        save()
        ...
        dialog().get_by_label('CPF / CNPJ').fill('11222333000181');save()
        page.get_by_role('button',name='Dados da mantenedora').click()
        expect(dialog().get_by_label('CPF / CNPJ')).to_have_value('11222333000181')
