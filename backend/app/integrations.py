...
from .integration_core import (seal,unseal,validate_connect_config,connection,connection_output,
    ConnectProvider,AsaasProvider,IntegrationFailure,enqueue)
...
def list_connections(db:DB,user:Actor,school:Scope):
    ...
    return [connection_output(x) for x in db.scalars(select(m.IntegrationConnection).where(m.IntegrationConnection.school_id==school.id))]
...
def save_connection(provider:str,data:s.ConnectionInput,db:DB,user:Actor,school:Scope,request:Request):
    ...
    if provider not in ('asaas','connect_api'):fail(404,'Provider não disponível.')
    if provider=='asaas':
        if data.config:fail(422,'A conexão ASAAS utiliza endpoints oficiais fixos; não recebe URLs arbitrárias.')
        config={}
    else:config=validate_connect_config(data.config)
    ...
    if data.enabled and provider=='connect_api' and not config['contract_confirmed']:fail(422,'Confira os caminhos e o contrato da sua Connect API antes de habilitar.')
    if obj:
        ...
        if provider=='connect_api' and any(obj.config.get(k)!=config.get(k) for k in ('base_url','instance')) and db.scalar(select(m.IntegrationJob.id).where(m.IntegrationJob.connection_id==obj.id,m.IntegrationJob.status.in_(['pending','processing','retry','uncertain'])).limit(1)):
            fail(409,'Resolva os envios pendentes antes de trocar host ou instância.')
...
def test_connection(provider:str,db:DB,user:Actor,school:Scope,request:Request):
    ...
    try:
        if provider=='connect_api':ConnectProvider(obj).state()
        elif provider=='asaas':AsaasProvider(obj).request('/customers',params={'limit':1})
        else:fail(404,'Provider não disponível.')
...
def send_message(data:s.SendMessage,db:DB,user:Actor,school:Scope,request:Request):
    require(user,'communications.send');lock_school(db,school.id)
    obj=scoped(db,m.Admission,data.admission_id,school.id);account=db.get(m.PortalAccount,obj.account_id)
    if not account.phone_verified or not account.whatsapp_opt_in:fail(409,'O responsável precisa verificar o telefone e autorizar os avisos por WhatsApp.')
    conn=connection(db,school.id,'connect_api')
    task=enqueue(db,school.id,'connect_text',{'number':account.phone,'text':data.text},f'manual-message:{school.id}:{data.client_key}',conn.id)
    audit(db,request,user,'connect.message.queued',obj,school.id,{'job_id':task.id})
...
async def webhook(provider:str,connection_id:str,request:Request,db:DB):
    if provider not in ('asaas','connect_api'):fail(404,'Webhook não encontrado.')
    ...
    header='asaas-access-token' if provider=='asaas' else 'x-connect-webhook-token'
    ...
    if provider=='asaas':
        ...
        if payload.get('instance')!=conn.config.get('instance'):fail(403,'Evento de outra instância recusado.')
        body=payload.get('data',{})
        if not isinstance(body,dict):fail(400,'Objeto data inválido.')
        key=body.get('key') or {}
        remote=str(body.get('messageId') or (key.get('id') if isinstance(key,dict) else '') or '')[:160]
        # Adaptador de callbacks explícito; não importa conversas ou instâncias remotas.
        event_id=str(payload.get('id') or hashlib.sha256(raw).hexdigest())[:200]
        if not remote:fail(400,'Identificador da mensagem ausente no callback.')
    ...
    if provider=='asaas':
        ...
        task=db.scalar(select(m.IntegrationJob).where(m.IntegrationJob.connection_id==conn.id,m.IntegrationJob.remote_id==remote,m.IntegrationJob.school_id==conn.school_id))
        status=str(body.get('status','')).lower()
        if task and status in ('sent','delivered','read','failed'):
            rank={'':0,'sent':1,'delivered':2,'read':3,'failed':-1}
            if status=='failed' and rank.get(task.delivery_status,0)<2 or rank.get(status,0)>rank.get(task.delivery_status,0):task.delivery_status=status
